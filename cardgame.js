...
function login() {
    ...
    for(let i = 0;i < 2; i++){ //this corresponds to the details array, this expresses that there are 2 sets of detail possibilities
        if (details[i][0] === username && details[i][1] === password){  //this makes sure that the username entered matches the username by the registered user
                ...
                document.getElementById("login-header").style.display="none"; //removes the element from the display
                document.getElementById("login-error-msg").style.display="none"; //removes the element from the display
        }else { console.log("Incorrect Username or Password"); //if details entered incorrectly then it tells the user that they have incorrectly entered their details
                document.getElementById("login-error-msg").style.display="inline-block"; //removes the element from the display
        }
    }
}
